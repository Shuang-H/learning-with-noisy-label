...
class PretrainedModel(object):
	...
	def train_model(self, dataloaders, criterion, lr, T_lr, num_epochs=25, reweight=False):
		self.lr = lr
		if reweight:
			self.T = Variable(torch.tensor(np.eye(self.num_classes)), requires_grad=True).cuda()
			self.T_lr = T_lr
		since = time.time()
		...
		for epoch in range(num_epochs):
			...
			if (epoch+1)%10 == 0:
				self.lr = self.lr * 0.5
				if reweight: self.T_lr = self.T_lr * 0.5
			...
			for phase in ['train', 'val', 'test']:
				...
				for inputs, labels in dataloaders[phase]:
					...
					with torch.set_grad_enabled(phase == 'train'):
						if reweight and phase == 'train':
							loss = criterion(outputs, labels, self.T)
						else:
						...
						if phase == 'train':
							...
							param_list = self.update_params(params_name, self.model.parameters(), param_grads, first_batch)
							if reweight and first_batch: # compute T
								X_val, y_val = iter(dataloaders['val']).next()
								X_val, y_val = X_val.cuda(), y_val.cuda()
								self.compute_T(inputs, labels, X_val, y_val, param_list)
								#print('T = ', self.T.data)
								first_batch = False
	...
	def compute_T(self, X_train, y_train, X_val, y_val, param_list):
		T_criterion = nn.CrossEntropyLoss()
		val_loss = T_criterion(self.model(X_val), y_val)
		param_grads_val = torch.autograd.grad(outputs=val_loss, inputs=self.model.parameters(), only_inputs=True)
		T_grad = torch.autograd.grad(outputs=param_list, inputs=self.T, grad_outputs=param_grads_val, only_inputs=True)

		self.T = self.T - self.T_lr * T_grad[0]
		self.T = self.normalize(self.T)
