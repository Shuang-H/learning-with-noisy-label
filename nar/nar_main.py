...
parser = argparse.ArgumentParser(description='NAR Training Model')
...
def load_data(input_size):
	...
	if args.data == 'Clothing_1M':
		...
		dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=args.batch_size, shuffle=True, num_workers=4) for x in ['train', 'val', 'test']}
		...
		val_loader = torch.utils.data.DataLoader(cifar_val, batch_size=args.batch_size, shuffle=True, num_workers=4)
		test_loader = torch.utils.data.DataLoader(cifar_test, batch_size=args.batch_size, shuffle=True, num_workers=4)
...
if __name__ == "__main__":
	...
	for lr in 10**np.linspace(-3, -1, 3):
		for T_lr in [0.1, 0.2, 0.3, 0.4]:
			print('lr =', lr, 'T_lr =', T_lr)
			model_ft, best_acc = pretrained_model.train_model(dataloaders_dict, criterion, lr, T_lr, num_epochs=args.epochs, reweight=True)
	...
	print('-' * 10)
	print('best_converge_lr =', best_lr, 'best_T_lr =', best_T_lr)
